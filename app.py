...
def save_route_html(route_list, graph, oCoord, dCoord):
    ...
    for point in range(0, len(locationlist)):
        flm.Marker(locationlist[point], popup=popup_list[point], tooltip=string[point]).add_to(graph_map)
        
    route_graph_map = ox.plot_route_folium(graph, route_list[1], route_map=graph_map, popup_attribute='length', route_color='#66ff33', route_opacity=0.8)    
    route_graph_map = ox.plot_route_folium(graph, route_list[0], route_map=graph_map, popup_attribute='length', route_opacity=1, route_width=2)
